...
export const CONFIG_MOBILE = {
    ...
    codec: 'avc1.42E01E' // Baseline Profile (iOS互換重視)
};

export const CONFIG_PC = {
    width: 540, height: 960, fps: 30, bitrate: 2_500_000, 
    codec: 'avc1.4D401F' // High Profile (PC安定版の指定)
};
...
export async function generateStampVideo(params, onProgress) {
    // ユーザーエージェント等で判断するか、paramsで指定
    const isMobile = /iPhone|iPad|Android/i.test(navigator.userAgent);
    const config = isMobile ? CONFIG_MOBILE : CONFIG_PC;
    
    return await executeEncoding(params, config, onProgress);
}

async function executeEncoding(params, config, onProgress) {
    ...
    for (let i = 0; i < stampFiles.length; i++) {
        ...
        while (stampTime < durationLimit) {
            // PC版の安定したキュー管理(2)とスマホの余裕(5)の間をとって調整
        ...
        await encoder.flush(); // スタンプごとにフラッシュしてメモリをクリーンに保つ
    }
